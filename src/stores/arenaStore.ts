...
import { StorageService, DEFAULT_SETTINGS, DEFAULT_WA_TEMPLATES, DEFAULT_17AN_TEMPLATE_COMPETITIONS, DEFAULT_COMMITTEES, FIX_BOJONG_LIO_COMMITTEES, FIX_BOJONG_LIO_RUNDOWNS, FIX_BOJONG_LIO_MONEY_TRANSACTIONS, FIX_BOJONG_LIO_RAB_ITEMS, DEFAULT_MONEY_TRANSACTIONS, DEFAULT_RAB_ITEMS, DEFAULT_RUNDOWN_ITEMS } from '../services/storage';
...
export const useArenaStore = defineStore('arena', {
  state: () => ({
    ...
    moneyTransactions: [] as MoneyTransaction[],
    rabItems: [] as RabItem[],
  ...
  actions: {
    init() {
      ...
      if (!data.committees || data.committees.length === 0) {
        this.committees = FIX_BOJONG_LIO_COMMITTEES;
        StorageService.set('17an_committees', this.committees);
      } else {
        this.committees = data.committees;
      }
      if (!data.moneyTransactions || data.moneyTransactions.length === 0) {
        this.moneyTransactions = FIX_BOJONG_LIO_MONEY_TRANSACTIONS;
        StorageService.set('17an_money_transactions', this.moneyTransactions);
      } else {
        this.moneyTransactions = data.moneyTransactions;
      }

      if (!data.rabItems || data.rabItems.length === 0) {
        this.rabItems = FIX_BOJONG_LIO_RAB_ITEMS;
        StorageService.set('17an_rab_items', this.rabItems);
      } else {
        this.rabItems = data.rabItems;
      }
      if (!data.rundownItems || data.rundownItems.length === 0) {
        this.rundownItems = FIX_BOJONG_LIO_RUNDOWNS;
        StorageService.set('17an_rundown_items', this.rundownItems);
      } else {
      ...
      this.history = [
        {
          ...
          action: 'System Flush (Clean App)',
          details: 'Sistem dibersihkan total dari seluruh data. Siap digunakan untuk acara baru.'
        }
      ];
      this.saveAll();
    },
    ...
    applyCompetitionTemplate(mode: 'append' | 'replace' = 'replace') {
      if (mode === 'replace') {
        this.competitions = [];
      }
      DEFAULT_17AN_TEMPLATE_COMPETITIONS.forEach(tmpl => {
        const exists = this.competitions.some(c => c.name.toLowerCase() === tmpl.name.toLowerCase());
        if (!exists) {
      ...
      this.logActivity('Template Lomba Dimuat', 'Memuat daftar template lomba Fix 17 Agustus 2026 | Bojong Lio.');
      this.saveAll();
    },
    ...
    deleteRegistration(id: string) {
      const reg = this.registrations.find(r => r.id === id);
      if (!reg) return;
      const part = this.participants.find(p => p.id === reg.participantId);
      const comp = this.competitions.find(c => c.id === reg.competitionId);

      this.registrations = this.registrations.filter(r => r.id !== id);
      this.scores = this.scores.filter(s => !(s.participantId === reg.participantId && s.competitionId === reg.competitionId));

      const winner = this.winners.find(w => w.competitionId === reg.competitionId);
      if (winner) {
        if (winner.firstPlaceId === reg.participantId) winner.firstPlaceId = undefined;
        if (winner.secondPlaceId === reg.participantId) winner.secondPlaceId = undefined;
        if (winner.thirdPlaceId === reg.participantId) winner.thirdPlaceId = undefined;
      }

      this.certificates = this.certificates.filter(c => !(c.participantId === reg.participantId && c.competitionId === reg.competitionId));

      this.logActivity('Registrasi Peserta Dihapus', `Registrasi ${reg.participantNumber} (${part?.name || 'Peserta'}) pada lomba ${comp?.name || 'Lomba'} telah dihapus.`);
      this.saveAll();
    },

    bulkDeleteRegistrations(ids: string[]) {
      let count = 0;
      ids.forEach(id => {
        const reg = this.registrations.find(r => r.id === id);
        if (reg) {
      ...
      if (count > 0) {
        this.logActivity('Penghapusan Peserta Massal', `Berhasil menghapus ${count} pendaftaran peserta.`);
        this.saveAll();
      }
    },

    deleteWinner(competitionId: string) {
      const comp = this.competitions.find(c => c.id === competitionId);
      this.winners = this.winners.filter(w => w.competitionId !== competitionId);
      if (comp) {
        comp.status = 'In Progress';
      }
      this.logActivity('Penetapan Juara Dihapus', `Penetapan juara untuk lomba ${comp?.name || competitionId} berhasil direset/dihapus.`);
      this.saveAll();
    },
    ...
    bulkAddCommitteeMembers(membersList: { name: string; role?: string; section?: string; phone?: string; email?: string; rtRw?: string }[], mode: 'append' | 'replace' = 'append') {
      if (mode === 'replace') {
        this.committees = [];
      }
      ...
      membersList.forEach((item) => {
        ...
        const newMember: CommitteeMember = {
          ...
          section: item.section?.trim() || 'Internal',
      ...
      this.logActivity('Bulk Edit Panitia', `Berhasil ${mode === 'replace' ? 'memperbarui seluruh' : 'menambahkan'} ${added.length} panitia.`);
      this.saveAll();
      return added;
    },

    // --- MONEY MANAGEMENT ACTIONS ---
    addMoneyTransaction(tx: Omit<MoneyTransaction, 'id' | 'createdAt'>) {
      const newTx: MoneyTransaction = {
        ...tx,
        id: 'tx_' + uuidv4().substring(0, 8),
        createdAt: new Date().toISOString()
      };
      this.moneyTransactions.unshift(newTx);
      this.logActivity('Tambah Transaksi Kas', `[${tx.type}] ${tx.category}: Rp ${tx.amount.toLocaleString('id-ID')} (${tx.description})`);
      this.saveAll();
      return newTx;
    },

    updateMoneyTransaction(id: string, updated: Partial<MoneyTransaction>) {
      const idx = this.moneyTransactions.findIndex(t => t.id === id);
      if (idx !== -1) {
        this.moneyTransactions[idx] = { ...this.moneyTransactions[idx], ...updated };
        this.logActivity('Edit Transaksi Kas', `Memperbarui transaksi ID ${id}`);
        this.saveAll();
      }
    },

    deleteMoneyTransaction(id: string) {
      const tx = this.moneyTransactions.find(t => t.id === id);
      if (tx) {
        this.moneyTransactions = this.moneyTransactions.filter(t => t.id !== id);
        this.logActivity('Hapus Transaksi Kas', `Menghapus transaksi: ${tx.description} (Rp ${tx.amount.toLocaleString('id-ID')})`);
        this.saveAll();
      }
    },

    // --- RAB MANAGEMENT ACTIONS ---
    addRabItem(item: Omit<RabItem, 'id' | 'totalEstimated'>) {
      const totalEstimated = item.quantity * item.pricePerUnit;
      const newItem: RabItem = {
        ...item,
        id: 'rab_' + uuidv4().substring(0, 8),
        totalEstimated
      };
      this.rabItems.push(newItem);
      this.logActivity('Tambah Item RAB', `[${item.section}] ${item.itemName} - Rp ${totalEstimated.toLocaleString('id-ID')}`);
      this.saveAll();
      return newItem;
    },

    updateRabItem(id: string, updated: Partial<RabItem>) {
      const idx = this.rabItems.findIndex(r => r.id === id);
      if (idx !== -1) {
        const qty = updated.quantity ?? this.rabItems[idx].quantity;
        const price = updated.pricePerUnit ?? this.rabItems[idx].pricePerUnit;
        this.rabItems[idx] = {
          ...this.rabItems[idx],
          ...updated,
          totalEstimated: qty * price
        };
        this.logActivity('Edit Item RAB', `Memperbarui item RAB: ${this.rabItems[idx].itemName}`);
        this.saveAll();
      }
    },

    deleteRabItem(id: string) {
      const item = this.rabItems.find(r => r.id === id);
      if (item) {
        this.rabItems = this.rabItems.filter(r => r.id !== id);
        this.logActivity('Hapus Item RAB', `Menghapus item RAB: ${item.itemName}`);
        this.saveAll();
      }
    },

    // --- RUNDOWN MANAGEMENT ACTIONS ---
    addRundownItem(item: Omit<RundownItem, 'id'>) {
      const newItem: RundownItem = {
        ...item,
        id: 'rd_' + uuidv4().substring(0, 8)
      };
      this.rundownItems.push(newItem);
      this.logActivity('Tambah Agenda Rundown', `${item.timeStart}-${item.timeEnd}: ${item.activity}`);
      this.saveAll();
      return newItem;
    },

    updateRundownItem(id: string, updated: Partial<RundownItem>) {
      const idx = this.rundownItems.findIndex(rd => rd.id === id);
      if (idx !== -1) {
        this.rundownItems[idx] = { ...this.rundownItems[idx], ...updated };
        this.logActivity('Edit Rundown', `Memperbarui acara: ${this.rundownItems[idx].activity}`);
        this.saveAll();
      }
    },

    deleteRundownItem(id: string) {
      const item = this.rundownItems.find(rd => rd.id === id);
      if (item) {
        this.rundownItems = this.rundownItems.filter(rd => rd.id !== id);
        this.logActivity('Hapus Rundown', `Menghapus acara rundown: ${item.activity}`);
        this.saveAll();
      }
    }
  }
