              ...
              <tbody class="divide-y divide-slate-100">
                ...
                >
                  ...
                  <td class="p-2.5 font-mono text-slate-700 text-[11px]">
                    <div v-if="item.cleanPhone" class="flex items-center gap-1.5 text-emerald-700 font-bold">
                      <span class="px-1.5 py-0.5 bg-emerald-100 text-emerald-800 rounded font-mono text-[10px] font-extrabold">+62</span>
                      <span>{{ item.formattedPhone }}</span>
                    </div>
                    <button
                      v-else
                      @click="editParticipantPhone(item)"
                      class="text-amber-700 bg-amber-50 hover:bg-amber-100 border border-amber-200 px-2 py-0.5 rounded font-bold text-[10px] flex items-center gap-1 transition-colors"
                    >
                      <i class="bi bi-telephone-plus"></i>
                      <span>+ Isi No WA (+62)</span>
                    </button>
                  </td>
                  <td class="p-2.5 text-center">
                    <span
...
import { renderBulkTemplate, generateWaLink, cleanPhoneNumber, formatPhoneDisplay } from '../services/whatsapp';
...
async function editParticipantPhone(item: RecipientItem) {
  const { value: newPhone } = await Swal.fire({
    title: `Nomor WA untuk ${item.participantName}`,
    text: 'Masukkan nomor WhatsApp peserta (dimulai 08... atau 62...):',
    input: 'text',
    inputValue: item.phone || '',
  ...
  if (newPhone !== undefined && newPhone.trim()) {
    store.updateParticipant(item.participantId, { phone: newPhone.trim() });
    Swal.fire({
      icon: 'success',
      title: 'Nomor WhatsApp Diperbarui!',
      text: `No WA ${item.participantName} diset menjadi +62 ${cleanPhoneNumber(newPhone.trim()).substring(2)}`,
      timer: 1500,
      showConfirmButton: false
    });
  }
...
function toggleSelectAll() {
  if (isAllSelected.value) {
    ...
    selectedIds.value = targetParticipants.value.filter(item => item.cleanPhone).map(item => item.id);
  }
}
