  ...
  <div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex border-b border-slate-200 gap-4 text-xs font-bold overflow-x-auto no-scrollbar">
      ...
      <button
        @click="activeTab = 'pwa'"
        class="pb-3 border-b-2 transition-colors flex items-center gap-1.5 whitespace-nowrap"
        :class="activeTab === 'pwa' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-800'"
      >
        <i class="bi bi-laptop-fill text-amber-500"></i>
        <span>Desktop Electron & Offline</span>
      </button>
        ...
        <div class="p-4 bg-slate-50 border border-slate-200 rounded-2xl space-y-3">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2.5">
              <div class="w-9 h-9 rounded-xl bg-red-600 text-white flex items-center justify-center text-lg font-bold shadow-xs">
                <i class="bi bi-translate"></i>
              </div>
              <div>
                <h3 class="text-xs font-black text-slate-900 uppercase tracking-wide">
                  Setelan Bahasa Sistem (Language Settings)
                </h3>
                <p class="text-[11px] text-slate-500">
                  Pilih bahasa utama tampilan sistem 17an Arena.
                </p>
              </div>
            </div>
            <span class="px-2.5 py-1 bg-red-100 text-red-800 font-extrabold text-[10px] rounded-full border border-red-200">
              8 Bahasa Tersedia
            </span>
          </div>
          ...
          <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-1">
            <button
              ...
              :class="form.language === lang.code ? 'bg-red-50 border-red-500 text-red-950 font-black shadow-xs ring-2 ring-red-500/20' : 'bg-white border-slate-200 text-slate-700 hover:border-slate-300 font-bold'"
            >
              <span class="text-base">{{ lang.flag }}</span>
              <div class="truncate">
                <div class="text-xs leading-tight truncate">{{ lang.name }}</div>
                <div class="text-[10px] text-slate-400 font-medium capitalize">{{ lang.nativeName }}</div>
              </div>
            </button>
          </div>
        </div>
    ...
    <!-- TAB: STATUS PWA & MODE OFFLINE LAPTOP -->
    <div v-if="activeTab === 'pwa'" class="space-y-6">
      <div class="bg-white rounded-3xl border border-slate-200 p-6 shadow-2xs space-y-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-5">
          <div class="space-y-1">
            <h3 class="text-lg font-black text-slate-900 flex items-center gap-2">
              <i class="bi bi-laptop-fill text-red-600"></i>
              <span>Progressive Web App (PWA) & Offline Engine</span>
            </h3>
            <p class="text-xs text-slate-500">
              Aplikasi 17an Arena dirancang khusus dengan arsitektur <strong>Offline First</strong> untuk berjalan mandiri di laptop/HP tanpa internet.
            </p>
          </div>
          ...
          <button
            ...
            class="px-5 py-2.5 bg-gradient-to-r from-red-600 to-amber-600 hover:from-red-700 hover:to-amber-700 text-white font-black text-xs rounded-xl shadow-md transition-all flex items-center justify-center gap-2 w-full sm:w-auto"
          >
            <i class="bi bi-download text-amber-300"></i>
            <span>Install Ke Laptop Sekarang</span>
          </button>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div class="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-1">
            <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Status Aplikasi PWA</span>
            <div class="flex items-center gap-2">
              <span
                class="w-3 h-3 rounded-full"
                :class="isPwaInstalled ? 'bg-emerald-500' : 'bg-amber-500 animate-pulse'"
              ></span>
              <span class="font-extrabold text-xs text-slate-900">
                {{ isPwaInstalled ? 'Terpasang (PWA App)' : 'Mode Browser Web' }}
              </span>
            </div>
            <p class="text-[11px] text-slate-500">
              {{ isPwaInstalled ? 'Aplikasi berjalan sebagai aplikasi desktop mandiri.' : 'Siap di-download ke laptop.' }}
            </p>
          </div>
          ...
          <div class="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-1">
            ...
            <div class="flex items-center gap-2">
              <i class="bi bi-cpu-fill text-emerald-600"></i>
              <span class="font-extrabold text-xs text-slate-900">
                {{ isSwRegistered ? 'Aktif & Melayani' : 'Menyiapkan ServiceWorker' }}
              </span>
            </div>
            <p class="text-[11px] text-slate-500">
              Setiap halaman & aset CSS/JS otomatis dicache untuk akses offline.
            </p>
          </div>

          <!-- Network Connection Status -->
          <div class="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-1">
            <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Koneksi Internet Jaringan</span>
            <div class="flex items-center gap-2">
              <i class="bi" :class="isOnline ? 'bi-wifi text-emerald-600' : 'bi-wifi-off text-amber-600'"></i>
              <span class="font-extrabold text-xs" :class="isOnline ? 'text-emerald-700' : 'text-amber-700'">
                {{ isOnline ? 'Online (Terhubung)' : 'Offline (Tanpa Internet)' }}
              </span>
            </div>
            <p class="text-[11px] text-slate-500">
              Aplikasi tetap 100% berfungsi meskipun internet mati total.
            </p>
          </div>

          <!-- Offline Storage Stats -->
          <div class="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-1">
            <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Penyimpanan Lokal</span>
            <div class="flex items-center gap-2">
            ...
            <p class="text-[11px] text-slate-500">
              Peserta, skor, sertifikat & TV Board aman tersimpan di perangkat.
            </p>
          </div>
        </div>

        <!-- ELECTRON DESKTOP 100% STANDALONE APP SECTION -->
        <div class="bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 text-white rounded-2xl p-6 space-y-5 border border-indigo-500/30 shadow-xl">
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 pb-4">
            <div class="flex items-center gap-3">
              <div class="w-12 h-12 rounded-2xl bg-indigo-600/30 border border-indigo-400/30 flex items-center justify-center text-2xl text-cyan-300">
                <i class="bi bi-window-desktop"></i>
              </div>
              <div>
                <div class="flex items-center gap-2">
                  <h4 class="font-black text-base text-white">
                    Electron Desktop Standalone App (100% Offline)
                  </h4>
                  <span class="px-2 py-0.5 bg-cyan-500/20 text-cyan-300 border border-cyan-500/40 rounded-full text-[10px] font-extrabold uppercase">
                    Zero Internet Needed
                  </span>
                </div>
                <p class="text-xs text-slate-300 mt-0.5">
                  Jalankan aplikasi sebagai program desktop native (.exe / Windows Installer / Mac / Linux) dengan sistem penyimpanan file lokal terisolasi.
                </p>
              </div>
            </div>
            ...
            <div class="flex items-center gap-2">
              <button
                @click="copyElectronCommand"
                class="px-4 py-2 bg-cyan-600 hover:bg-cyan-700 text-white font-extrabold text-xs rounded-xl shadow-sm transition-all flex items-center gap-1.5"
              >
                <i class="bi bi-clipboard-check"></i>
                <span>Salin Perintah Build</span>
              </button>
            </div>
          </div>

          <!-- Step by step to run Electron Locally & Offline -->
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
            <!-- Step 1 -->
            <div class="bg-white/5 border border-white/10 rounded-xl p-4 space-y-2">
              <div class="flex items-center gap-2 text-cyan-300 font-extrabold">
                <span class="w-6 h-6 rounded-lg bg-cyan-500/20 text-cyan-300 flex items-center justify-center font-mono">1</span>
                <span>Jalankan Sekali Klik (.bat)</span>
              </div>
              <p class="text-slate-300 text-[11px] leading-relaxed">
                Di Windows, cukup dobel klik file:
              </p>
              <div class="bg-black/50 p-2 rounded-lg font-mono text-[11px] text-amber-300 border border-white/10">
                start-offline-windows.bat
              </div>
              <p class="text-[10px] text-slate-400">
                Otomatis menginstall, membuild aset lokal, dan membuka aplikasi desktop.
              </p>
            </div>

            <!-- Step 2 -->
            <div class="bg-white/5 border border-white/10 rounded-xl p-4 space-y-2">
              <div class="flex items-center gap-2 text-indigo-300 font-extrabold">
                <span class="w-6 h-6 rounded-lg bg-indigo-500/20 text-indigo-300 flex items-center justify-center font-mono">2</span>
                <span>Jalankan via Terminal/CMD</span>
              </div>
              <p class="text-slate-300 text-[11px] leading-relaxed">
                Jalankan perintah ini di Command Prompt / Terminal:
              </p>
              <div class="bg-black/50 p-2 rounded-lg font-mono text-[11px] text-emerald-300 border border-white/10 space-y-1">
                <div>npm run build</div>
                <div>npx electron .</div>
              </div>
              <p class="text-[10px] text-slate-400">
                Buka antarmuka desktop native secara langsung.
              </p>
            </div>

            <!-- Step 3 -->
            <div class="bg-white/5 border border-white/10 rounded-xl p-4 space-y-2">
              <div class="flex items-center gap-2 text-emerald-300 font-extrabold">
                <span class="w-6 h-6 rounded-lg bg-emerald-500/20 text-emerald-300 flex items-center justify-center font-mono">3</span>
                <span>Buat File Installer .exe / Portable</span>
              </div>
              <p class="text-slate-300 text-[11px] leading-relaxed">
                Paketkan menjadi file .exe siap pakai untuk flashdisk:
              </p>
              <div class="bg-black/50 p-2 rounded-lg font-mono text-[11px] text-amber-300 border border-white/10">
                npm run dist:win
              </div>
              <p class="text-[10px] text-slate-400">
                File installer/portable akan tersimpan di folder <code class="text-white">/release</code>.
              </p>
            </div>
          </div>
        </div>

        <!-- How to Install PWA Instructions Guide -->
        <div class="bg-gradient-to-r from-slate-900 to-slate-800 text-white rounded-2xl p-6 space-y-4">
          <h4 class="font-extrabold text-sm text-white flex items-center gap-2">
            <i class="bi bi-question-circle-fill text-amber-400"></i>
            <span>Cara Install Aplikasi ke Laptop & HP (Panduan Lengkap)</span>
          </h4>

          <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
            <!-- Windows / Mac Chrome & Edge -->
            <div class="bg-white/10 rounded-xl p-4 space-y-2 border border-white/10">
              <h5 class="font-bold text-amber-300 flex items-center gap-1.5">
                <i class="bi bi-windows"></i> Windows & Mac (Google Chrome / Edge)
              </h5>
              <ol class="list-decimal list-inside space-y-1 text-slate-300 text-[11px] leading-relaxed">
                <li>Klik tombol <strong>"Install App PWA"</strong> di bilah navigasi atas atau banner merah di atas.</li>
                <li>Atau klik ikon <strong>Install / Komputer (+)</strong> di bilah alamat browser kanan atas.</li>
                <li>Konfirmasi klik <strong>"Install"</strong>. PWA akan langsung muncul di Desktop & Menu Start laptop Anda!</li>
              </ol>
            </div>
            ...
            <div class="bg-white/10 rounded-xl p-4 space-y-2 border border-white/10">
              <h5 class="font-bold text-emerald-300 flex items-center gap-1.5">
                <i class="bi bi-android2"></i> Android (Chrome / Brave / Edge)
              </h5>
              <ol class="list-decimal list-inside space-y-1 text-slate-300 text-[11px] leading-relaxed">
                <li>Buka link aplikasi di Google Chrome HP Android Anda.</li>
                <li>Tekan titik tiga <strong>(⋮)</strong> di sudut kanan atas Chrome.</li>
            ...
            <!-- iOS / Mac Safari -->
            <div class="bg-white/10 rounded-xl p-4 space-y-2 border border-white/10">
              <h5 class="font-bold text-sky-300 flex items-center gap-1.5">
                <i class="bi bi-apple"></i> iPhone & iPad (Safari)
              </h5>
              <ol class="list-decimal list-inside space-y-1 text-slate-300 text-[11px] leading-relaxed">
                <li>Buka aplikasi menggunakan browser <strong>Safari</strong> di iOS.</li>
                <li>Tekan tombol <strong>Bagikan / Share (ikon kotak panah ke atas)</strong>.</li>
...
import { isOnline, deferredInstallPrompt, isPwaInstalled, promptInstallPwa, isSwRegistered } from '../services/pwa';
import Swal from 'sweetalert2';

const store = useArenaStore();
const activeTab = ref<'general' | 'competition-template' | 'wa-templates' | 'pwa' | 'danger'>('general');
...
function copyElectronCommand() {
  const textToCopy = `npm run build && npx electron .`;
  navigator.clipboard.writeText(textToCopy).then(() => {
    Swal.fire({
      ...
      text: 'Buka Terminal/CMD dan jalankan "npm run build && npx electron ."',
      timer: 2500,
...
const languageOptions = [
  ...
  { code: 'ja', name: 'Jepang', nativeName: '日本語', flag: '🇯🇵' },
  { code: 'ko', name: 'Korea', nativeName: '한국어', flag: '🇰🇷' },
  { code: 'zh', name: 'China', nativeName: '中文', flag: '🇨🇳' },
  { code: 'ar', name: 'Arab', nativeName: 'العربية', flag: '🇸🇦' },
  { code: 'de', name: 'Jerman', nativeName: 'Deutsch', flag: '🇩🇪' },
  { code: 'nl', name: 'Belanda', nativeName: 'Nederlands', flag: '🇳🇱' }
];
...
onMounted(() => {
  ...
  form.language = store.settings.language || 'id';
});
