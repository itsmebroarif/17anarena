  ...
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      ...
      <div class="flex flex-wrap items-center gap-2 w-full sm:w-auto">
        <button
          @click="showScorePaperModal = true"
          ...
          title="Formulir Kertas Catatan Lapangan & Bracket Shaker Tim"
        >
        ...
        <button
          ...
          class="px-3.5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold text-xs rounded-xl shadow-xs transition-colors flex items-center justify-center gap-1.5"
          title="Formulir cetak A4 Lomba 17-an Bojong Lio & Send to WhatsApp"
        >
          <i class="bi bi-file-earmark-pdf-fill"></i>
          <span>📄 Form A4 Lomba</span>
        </button>
        ...
        <button
          ...
          title="Setel otomatis daftar lomba dengan 8 cabang template Fix Bojong Lio"
        >
          <i class="bi bi-trophy-fill"></i>
          <span>Muat Template Bojong Lio</span>
        </button>
      ...
      <div class="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden border border-slate-200">
        ...
        <form @submit.prevent="saveForm" class="p-5 space-y-4 max-h-[80vh] overflow-y-auto">
          ...
          <div v-if="!editingCompId" class="bg-amber-50 p-3 rounded-xl border border-amber-200 text-xs text-amber-900">
            <label class="font-bold block mb-1">Gunakan Preset Lomba Bojong Lio 17-an:</label>
            <select
            ...
            >
              <option value="">-- Pilih Template Preset Lomba Bojong Lio --</option>
              <optgroup label="👶 Lomba Anak-anak (Bojong Lio)">
                <option value="Makan Krupuk">🍘 Makan Krupuk (Individu)</option>
                <option value="Ngambil Karet di Tepung">🍥 Ngambil Karet di Tepung (Individu)</option>
                <option value="Masukin Paku Ke Botol Pake Spion">🪞 Masukin Paku Ke Botol Pake Spion (Individu)</option>
                <option value="Masukin Bendera">🚩 Masukin Bendera (Individu)</option>
                <option value="Bola Kampung">⚽ Bola Kampung (Tim 12 Orang | 4v4 2 Tim Cup)</option>
                <option value="Balap Karung">🥔 Balap Karung (Individu)</option>
              </optgroup>
              <optgroup label="👨 Lomba Dewasa (Bojong Lio)">
                <option value="Suap Pisang">🍌 Suap Pisang (Tim | 2 Orang)</option>
                <option value="Tarik Tambang">🪢 Tarik Tambang (Tim | 5 Orang)</option>
              </optgroup>
            </select>
          </div>
    ...
    <BojongLioPrintWaModal :is-open="showA4Modal" @close="showA4Modal = false" />

    <!-- Form Kertas Catatan Lomba & Bracket Shaker Modal -->
    <CompetitionScorePaperModal :is-open="showScorePaperModal" @close="showScorePaperModal = false" />
  </div>
</template>
...
const showA4Modal = ref(false);
const showScorePaperModal = ref(false);
...
function applyPreset(e: Event) {
  ...
  switch (val) {
    case 'Makan Krupuk':
      form.name = 'Makan Krupuk';
      form.category = 'Anak-anak';
      form.prefix = 'KRPK';
      form.description = 'Lomba makan kerupuk gantung tanpa menyentuh dengan tangan (Individu).';
      form.location = 'Bojong Lio (Lapangan Utama)';
      break;
    case 'Ngambil Karet di Tepung':
      form.name = 'Ngambil Karet di Tepung';
      form.category = 'Anak-anak';
      form.prefix = 'KRTG';
      form.description = 'Lomba mengambil karet di dalam nampan berisi tepung menggunakan mulut (Individu).';
      form.location = 'Bojong Lio (Lapangan Utama)';
      break;
    case 'Masukin Paku Ke Botol Pake Spion':
      form.name = 'Masukin Paku Ke Botol Pake Spion';
      form.category = 'Anak-anak';
      form.prefix = 'SPON';
      form.description = 'Lomba memasukkan paku terikat tali ke dalam botol dengan kaca spion (Individu).';
      form.location = 'Bojong Lio (Area Panggung)';
      break;
    case 'Masukin Bendera':
      form.name = 'Masukin Bendera';
      form.category = 'Anak-anak';
      form.prefix = 'BNDR';
      form.description = 'Lomba memindahkan bendera Merah Putih kecil ke dalam botol (Individu).';
      form.location = 'Bojong Lio (Lapangan Utama)';
      break;
    case 'Bola Kampung':
      form.name = 'Bola Kampung';
      form.category = 'Anak-anak';
      form.prefix = 'BOLA';
      form.description = 'Lomba sepak bola kampung (Tim 12 Orang | 4v4 2 Tim Cup).';
      form.location = 'Bojong Lio (Lapangan Bola)';
      break;
    case 'Balap Karung':
      form.name = 'Balap Karung';
      form.category = 'Anak-anak';
      form.prefix = 'KRNG';
      form.description = 'Lomba balap karung tradisional anak-anak (Individu).';
      form.location = 'Bojong Lio (Lapangan Utama)';
      break;
    case 'Suap Pisang':
      form.name = 'Suap Pisang';
      form.category = 'Dewasa';
      form.prefix = 'SPSG';
      form.description = 'Lomba menyuapi pisang dengan mata tertutup berpasangan 2 orang (Dewasa).';
      form.location = 'Bojong Lio (Panggung Utama)';
      break;
    case 'Tarik Tambang':
      form.name = 'Tarik Tambang';
      form.category = 'Dewasa';
      form.prefix = 'TMBG';
      form.description = 'Lomba adu kekuatan tarik tambang antar tim 5 orang (Dewasa).';
      form.location = 'Bojong Lio (Lapangan Utama)';
      break;
  }
}
